...
import { $Enums as Role} from '@prisma/client';
...
export class AuthService {
    ...
    async register(username: string, email: string, password: string): Promise<string> {
        ...
        const user = await this.prisma.user.create({
            data: {
                ...
                role: Role.userRole.SUPERADMIN, // Anda bisa ubah default role ini
            },
        });
    ...
    async login(username: string, password: string): Promise<{ access_token: string, refresh_token: string, role: string, id:number}> {
        ...
        const tokens = await this.getTokens(user.id_user, user.username, user.role);
