      ...
      <main class="editor-column">
        <div class="editor-box-container">
          ...
          <div class="editor-actions">
            ...
            <button class="btn-primary" @click="kirimUndangan" :disabled="isSubmitting">{{ isSubmitting ? 'Mengirim...' : 'Kirim Undangan' }}</button>
          </div>
        </div>
      </main>
...
const fetchPeserta = async () => {
  ...
  try {
    const [resMhs, resTamu] = await Promise.all([
      axios.get(`${API_URL}/api/mahasiswa`, { withCredentials: true }),
      axios.get(`${API_URL}/api/tamu`, { withCredentials: true })
    ]);
...
const kirimUndangan = async () => {
  ...
  try {
    await axios.post(`${API_URL}/invitation/queue`, {
...
.search-box { flex: 1; display: flex; align-items: center; background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 20px; padding: 0.5rem 1rem; min-width: 150px; }
.search-box input { 
  border: none; outline: none; width: 100%; margin-left: 0.5rem; background: transparent; 
  color: #333; font-weight: 500; /* Text Color Fix */
}
...
.input-subject { 
  width: 100%; padding: 0.75rem; border: 1px solid #e0f7fa; background: #f0fbff; border-radius: 8px; outline: none; box-sizing: border-box; 
  color: #000 !important; font-weight: 500; /* Hitam Pekat */
}
.input-subject::placeholder { color: #a0a0a0; }

.editor-group { flex: 1; display: flex; flex-direction: column; }
.editor-content-area { 
  flex: 1; background: #f0fbff; border: 1px solid #e0f7fa; border-radius: 12px; padding: 1rem; resize: none; font-family: inherit; font-size: 1rem; outline: none; width: 100%; box-sizing: border-box; min-height: 200px;
