...
const totalUndangan = ref(250);
const totalHadir = ref(175);
const totalBelumHadir = computed(() => totalUndangan.value - totalHadir.value);
const BASE_URL = 'api/'
...
const tableData = ref([
    { nim: '222011001', nama: 'Ahmad Fauzi', jurusan: 'TI', prodi: 'D4', status: 'hadir', waktu: '08:30' },
    { nim: '222011002', nama: 'Siti Nurhaliza', jurusan: 'SI', prodi: 'D3', status: 'belum', waktu: null },
    { nim: '222011003', nama: 'Budi Santoso', jurusan: 'TI', prodi: 'D4', status: 'hadir', waktu: '08:45' },
    { nim: '222011004', nama: 'Rina Wijaya', jurusan: 'SI', prodi: 'D4', status: 'belum', waktu: null },
    { nim: '222011005', nama: 'Doni Prasetyo', jurusan: 'TI', prodi: 'D3', status: 'hadir', waktu: '09:00' },
]);


const filteredData = computed(() => {
    return tableData.value.filter(item => {
        const matchSearch = item.nim.includes(searchQuery.value) ||
            item.nama.toLowerCase().includes(searchQuery.value.toLowerCase());
        const matchJurusan = !filterJurusan.value || item.jurusan.toLowerCase() === filterJurusan.value;
        const matchProdi = !filterProdi.value || item.prodi.toLowerCase() === filterProdi.value;
        return matchSearch && matchJurusan && matchProdi;
    });
});
...
const handleManualInput = () => {
    console.log('Manual input:', manualNim.value);
};
...
const handleScan = async (value) => {
    ...
    console.log(value)
    try {
        const res = await api.post(`${BASE_URL}scan`, { qr: String('2d9f089876dbf5e4a12e9dd1b32b3ed4') });
