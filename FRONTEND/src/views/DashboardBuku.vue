        ...
        <div class="relative z-20 px-8 md:px-12 py-12 w-full flex flex-col md:flex-row items-center gap-12">
          ...
          <div class="flex-1 text-left space-y-6">
            ...
            <p class="text-lg md:text-xl text-gray-200 leading-relaxed max-w-2xl font-light">
              Kelola data mahasiswa, penyusunan buku wisuda, dan dokumen kelengkapan dengan mudah dan efisien dalam satu tampilan.
            </p>
            ...
            <div class="relative bg-white/10 backdrop-blur-sm border border-white/20 rounded-[32px] p-2 shadow-2xl transition duration-500">
              <div class="rounded-[24px] overflow-hidden h-[320px] relative bg-gray-800">
                 ...
                 <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                    <span v-for="(slide, i) in slides" :key="slide.id" class="w-2 h-2 rounded-full transition-all" :class="i === activeIndex ? 'bg-blue-500 w-6' : 'bg-white/50'" />
                 </div>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-[32px] border border-gray-100 shadow-lg px-8 py-8 flex flex-col min-h-[380px] hover:translate-y-[-5px] transition-all duration-300">
              <div class="flex justify-between items-start mb-6">
                ...
                <button @click="goToPage('/input-file')" class="text-sm font-semibold text-[#2366d1] bg-blue-50 px-4 py-2 rounded-lg hover:bg-blue-100 transition">
                  Detail Data ➝
                </button>
              </div>
              ...
              <div class="flex flex-col items-center justify-center flex-1">
                <div class="relative w-56 h-56 md:w-64 md:h-64">
                  <div class="w-full h-full rounded-full transition-all duration-1000" :style="donutTemplateStyle"></div>
                  <div class="absolute inset-8 bg-white rounded-full flex flex-col items-center justify-center shadow-[inset_0_0_20px_rgba(0,0,0,0.05)]">
                    ...
                    <span class="text-5xl font-extrabold text-[#202020] mt-1">{{ formatNumber(totalTemplateBuku) }}</span>
                  </div>
                </div>
                
                <div class="mt-8 flex flex-wrap justify-center gap-3 text-xs font-medium">
                  <div v-for="item in templateByJenis" :key="item.jenis" class="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-200">
                    <span class="w-3 h-3 rounded-full" :style="{ backgroundColor: item.color }"></span>
                    <span class="text-gray-600">{{ item.jenis }}: <b class="text-gray-900">{{ formatNumber(item.jumlah) }}</b></span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="space-y-5">
                  <div v-for="row in mahasiswaByProdiWithPercent" :key="row.prodi" class="group">
                    <div class="flex justify-between items-end mb-2">
                      ...
                      <span class="text-xs font-semibold text-[#2366d1] bg-blue-50 px-2 py-0.5 rounded">{{ formatNumber(row.jumlah) }} Mhs</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                      <div class="h-full bg-[#377dff] rounded-full transition-all duration-1000 group-hover:bg-[#2e3e85]" :style="{ width: row.percent + '%' }"></div>
                    </div>
                  </div>
