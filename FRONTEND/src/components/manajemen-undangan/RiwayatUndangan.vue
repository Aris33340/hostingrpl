  ...
  <div class="tab-content">
    ...
    <div class="toolbar">
      <div class="search-container">
        ...
        <input type="text" placeholder="Cari Nama, Email, atau Folder" v-model="searchQuery" />
      </div>
        ...
        <tbody>
          ...
          <tr v-for="(item, index) in filteredData" :key="item.id">
            ...
            <td class="action-cell">
              <button v-if="item.statusCode === 0" class="btn-retry" @click="retryEmail(item.id)"
                :disabled="isRetrying">
                ↻ Kirim Ulang
              </button>
            </td>
...
const fetchData = async (isBackground = false) => {
  ...
  try {
    const response = await mainApi.get('invitation');
...
const retryEmail = async (id) => {
  ...
  try {
    await mainApi.put(`invitation/retry/${id}`, {});
